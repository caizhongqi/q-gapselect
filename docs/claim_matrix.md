...
| S-14 | Direct-phase resource records account for the dense NumPy QFT matrix and comparator workspace. | CODE-SANITY | Retained, expanded-comparator, dense-QFT, estimated-peak, and byte fields with budget-block tests | Preserve operation-specific execution semantics and distinguish logical IR from hardware | Peak memory omits a simultaneously live array or gate/depth is called compiled hardware | The artifact reports conservative exact-simulator allocation and logical circuit-IR counts. |

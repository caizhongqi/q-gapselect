#!/usr/bin/env python3
"""Run variable-time stopping-transducer skeleton audits."""
...
DEFAULT_CONFIG = REPOSITORY / "configs" / "stopping_time_transducer.json"
DEFAULT_OUTPUT = REPOSITORY / "artifacts" / "stopping_time_transducer.json"
ARTIFACT_TYPE = "q_gapselect_stopping_time_transducer"
...
def _git_provenance() -> dict[str, object]:
    ...
    return {
        ...
        "source_status_capture": "before_stopping_transducer_execution",
    }
        ...
        cases.append(
            {
                ...
                "max_statevector_dimension": _integer(
                    case.get("max_statevector_dimension", 262144),
...
def _run_record(case: Mapping[str, object], m: int) -> dict[str, object]:
    ...
    return {
        ...
        "serial_full_history_cost_per_branch": (
        ...
        "variable_over_serial_per_branch": summary.variable_over_serial_per_branch,
        "output_fraction": summary.output_fraction,
...
def _run_case(case: Mapping[str, object]) -> dict[str, object]:
    ...
    slopes = {
        ...
        for field in (
            "serial_full_history_cost_per_branch",
            "coherent_branch_rms_cost",
            "all_branch_rms_search_proxy",
            "variable_over_serial_per_branch",
            "output_fraction",
            "active_fraction",
        )
    }
    return {
        ...
        "summary": {
            ...
            "all_variable_over_serial_at_most_one": all(
                float(record["variable_over_serial_per_branch"]) <= 1.0
                for record in records
            ),
            "last_variable_over_serial_per_branch": records[-1][
                "variable_over_serial_per_branch"
            ],
            "last_output_fraction": records[-1]["output_fraction"],
            "executed_quantum_algorithm": False,
            "asymptotic_theorem_claimed": False,
            "interpretation": (
                "variable-time stopping relation skeleton with exact-state "
                "compute/phase/uncompute traces"
            ),
        },
...
def run_stopping_audit(config: Mapping[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "claim_boundaries": {
            "supports": [
                "first-output stopping relation generated from charged finite-QPE predicates",
                "exact-state compute/phase/uncompute traces for small stopping fixtures",
                "serial versus branch-RMS stopping-cost ledgers",
            ],
            "does_not_support": [
                "a proved variable-time coherent transducer upper bound",
        ...
        "summary": {
            ...
            "all_variable_over_serial_at_most_one": all(
                bool(result["summary"]["all_variable_over_serial_at_most_one"])
                for result in case_results
            ),
            "executed_quantum_algorithm": False,
            "asymptotic_theorem_claimed": False,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = run_stopping_audit(config)
        output = write_report(report, args.output)
    except (OSError, TypeError, ValueError) as error:
        raise SystemExit(f"stopping-time transducer error: {error}") from error
    print(
        "wrote "
        f"{report['summary']['total_records']} stopping-time transducer records "
        f"to {output}"
    )
    print(f"claim_status={report['claim_status']}")
    print(
        "This is a stopping relation skeleton, not a proved CCF-A-level "
        "variable-time theorem."
    )
    return 0

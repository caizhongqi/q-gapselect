#!/usr/bin/env python3
"""Freeze CIFAR-100 train/test arrays from the Hugging Face mirror."""
...
def _write_split(dataset, *, image_path: Path, label_path: Path) -> dict[str, object]:
    labels = np.asarray(dataset["fine_label"], dtype=np.int64)
    ...
    for index, sample in enumerate(dataset):
        ...
        if values.shape != (32, 32, 3):
            raise ValueError(f"unexpected CIFAR-100 image shape {values.shape}")
        images[index] = values
...
def main() -> int:
    ...
    cache = root / "hf-cache"
    train = load_dataset("uoft-cs/cifar100", split="train", cache_dir=str(cache))
    test = load_dataset("uoft-cs/cifar100", split="test", cache_dir=str(cache))
    if len(train) != 50000 or len(test) != 10000:
        raise RuntimeError("unexpected CIFAR-100 split size")
    metadata = {
        "artifact_type": "qcollide_cifar100_hf_raw_snapshot",
        "schema_version": 1,

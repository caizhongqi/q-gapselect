#!/usr/bin/env python3
"""Run variable-time charged activity-history alignment audits."""
...
DEFAULT_CONFIG = REPOSITORY / "configs" / "variable_time_charged_history.json"
DEFAULT_OUTPUT = REPOSITORY / "artifacts" / "variable_time_charged_history.json"
ARTIFACT_TYPE = "q_gapselect_variable_time_charged_history"
...
def _git_provenance() -> dict[str, object]:
    ...
    return {
        ...
        "source_status_capture": "before_variable_time_charged_execution",
    }
...
def _run_case(case: Mapping[str, object]) -> dict[str, object]:
    ...
    slopes = {
        field: variable_time_charged_history_loglog_slope(records, field)
        for field in SLOPE_FIELDS
    }
    gates = Counter(record.novelty_gate for record in records)
    strongest = Counter(record.min_valid_baseline_name for record in records)
    last = records[-1]
    return {
        "name": case["name"],
        ...
        "summary": {
            ...
            "last_point_serial_over_candidate": last.charged_serial_over_candidate,
            ...
            "interpretation": (
                "charged finite-QPE costs aligned with the unknown-boundary "
                "history target; open gates are proof targets"
            ),
        },
...
def run_variable_time_charged(config: Mapping[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "claim_boundaries": {
            "supports": [
                "mainline alignment of finite-QPE charged predicates with the "
                "unknown-boundary target",
                "same-interface proxy comparison against serial, variable-time "
                "rebuilt, Grover activity, independent, and coarse+BAI baselines",
            ],
            "does_not_support": [
                "a proved variable-time coherent transducer",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = run_variable_time_charged(config)
        output = write_report(report, args.output)
    except (OSError, TypeError, ValueError) as error:
        raise SystemExit(f"variable-time charged history error: {error}") from error
    print(
        "wrote "
        f"{report['summary']['total_records']} variable-time charged records "
        f"to {output}"
    )
    print(f"claim_status={report['claim_status']}")
    print(
        "This is a mainline alignment audit, not a proved CCF-A-level quantum "
        "advantage theorem."
    )
    return 0
